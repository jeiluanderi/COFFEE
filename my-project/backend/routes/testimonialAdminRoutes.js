...
// POST /api/admin/testimonials (Public)
router.post('/', async (req, res) => {
  const { customer_name, profession, quote, image_url } = req.body;
  try {
    const newTestimonial = await pool.query(
      `INSERT INTO testimonials (customer_name, profession, quote, image_url, status)
...
router.put('/:id', authenticateToken, authorizeRoles('admin'), async (req, res) => {
  const { id } = req.params;
  const { status, customer_name, profession, quote, image_url } = req.body;

  try {
    let updatedTestimonial;

    if (status && !customer_name && !profession && !quote && !image_url) {
      // Case 1: Admin only updating status (approve/reject)
      updatedTestimonial = await pool.query(
        `UPDATE testimonials 
      ...
      // Case 2: Admin editing full testimonial
      updatedTestimonial = await pool.query(
        `UPDATE testimonials 
         SET customer_name = $1, profession = $2, quote = $3, image_url = $4, status = $5, updated_at = NOW()
...
router.delete('/:id', authenticateToken, authorizeRoles('admin'), async (req, res) => {
  const { id } = req.params;
  try {
    const deletedTestimonial = await pool.query(
