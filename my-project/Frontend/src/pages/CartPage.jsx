...
import { useAuth } from '../context/AuthContext.jsx'; // Import the AuthContext hook
import { Link } from 'react-router-dom';
import axios from 'axios'; // Import axios for API calls

const CartPage = () => {
    // All hooks must be called at the top level of the component, before any early returns.
    ...
    const { user, isLoggedIn, token } = useAuth(); // Get user, login status, and token from AuthContext

    // State for the checkout form, now only for address and phone
    ...
    // Determine the backend URL based on the environment (Vite uses import.meta.env)
    ...
    // Helper function to safely get a numeric price
    ...
    const handleCheckoutChange = (e) => {
        ...
        setCheckoutData(prevData => ({ ...prevData, [id]: value }));
    };
    ...
    const handleCheckoutSubmit = async (e) => {
        ...
        // We now get the token directly from useAuth context, which is more reliable.
        if (!isLoggedIn || !token) {
            setMessage('Authentication token not found. Please log in again to place an order.');
        ...
        // Ensure user details are available from AuthContext
        ...
        try {
            // Construct the order data object to be sent to the backend
            const orderPayload = {
                customerName: user.username, // Use username from context
                customerEmail: user.email,   // Use email from context
                totalPrice: totalPrice,
                cartItems: cartItems.map(item => ({
                    id: item.id, // Ensure this matches backend expected field like coffee_id
                    quantity: item.quantity,
                    price: getNumericPrice(item.price) // Use the numeric price
                })),
            ...
            // If the order is successful, display a success message and clear the cart
            setMessage(response.data.message || 'Order placed successfully!');
            clearCart();
            setCheckoutData({ address: '', phone: '' }); // Clear the form
    ...
    return (
        <>
            <PageHeader title="Your Shopping Cart" activePage="Cart" />
